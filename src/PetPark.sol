    ...
    function borrow(uint8 age, uint8 gender, uint8 animalType) public{
       ...
       if(borrowers[msg.sender].borrower == msg.sender){
           require(borrowers[msg.sender].age == age &&  borrowers[msg.sender].gender == gender, "Invalid Age or Gender!");
       }
       require(borrowers[msg.sender].borrowed == false, "Already adopted a pet");
    ...
    function femaleAsBorrower(uint8 age, uint8 animalType) internal pure {
        uint8 catValue = 2;
        if(age < 40)
        require(animalType != catValue, "Invalid animal for women under 40");
    }
